...
/// The waiters table (`job_waiters`): a job parked until another is
/// terminal. Every write here is on the caller's `op`; see
/// `finalizer.rs::wake_waiters_in_op` for the consumer.
impl JobRepo {
    ...
    pub(super) async fn insert_waiters_in_op(
        &self,
        op: &mut (impl es_entity::AtomicOperation + ?Sized),
        callees: &[JobId],
        waiters: &[JobId],
        ...
        sqlx::query!(
            ...
            SELECT t.job_id, t.waiter_job_id, COALESCE($3, NOW())
    ...
    /// Register `waiter` on a callee that already exists: `true` if the
    /// callee is live and the row was written, `false` if it is terminal
    /// (nothing written — the caller reads the outcome instead of waiting).
    ///
    /// Takes `FOR SHARE` on the callee's execution row, which is what makes
    /// the answer honest: the finalizer that deletes that row takes `FOR
    /// UPDATE` on it first, so this either sees the row and blocks that
    /// finalizer until the waiter is committed (and so woken), or sees it
    /// already gone. Without the lock a registration could commit just after
    /// the wake ran and never be consumed.
    pub(super) async fn register_waiter_on_live_in_op(
        &self,
        op: &mut (impl es_entity::AtomicOperation + ?Sized),
        callee: JobId,
        waiter: JobId,
        ...
        let row = sqlx::query!(
            ...
            RETURNING job_id
            "#,
            uuid::Uuid::from(callee),
            uuid::Uuid::from(waiter),
            op.maybe_now(),
        )
        .fetch_optional(op.as_executor())
        .await?;
    ...
    /// Mark every un-woken wait on `callees` as woken at `now` and return
    /// the distinct waiters. The mark, not a delete, is what survives a
    /// waiter that cannot be moved right now (still running): its own park
    /// write finds the mark (`take_woken_marks_in_op`).
    pub(super) async fn mark_waiters_woken_in_op(
        &self,
        op: &mut (impl es_entity::AtomicOperation + ?Sized),
        callees: &[uuid::Uuid],
        ...
        let rows = sqlx::query!(
            ...
            WHERE job_id = ANY($1) AND woken_at IS NULL
    ...
    /// Delete the waits of `waiters` on `callees` — a wake that moved the
    /// waiter has consumed them.
    pub(super) async fn consume_waiters_in_op(
        &self,
        op: &mut (impl es_entity::AtomicOperation + ?Sized),
        callees: &[uuid::Uuid],
        waiters: &[uuid::Uuid],
        ...
        sqlx::query!(
            "DELETE FROM job_waiters WHERE job_id = ANY($1) AND waiter_job_id = ANY($2)",
            callees,
            waiters,
        )
        .execute(op.as_executor())
    ...
    /// The waiters among `ids` that a wake marked while they were running:
    /// deletes those marks and returns the distinct waiters, for the park
    /// write to land due instead of at the deadline they asked for.
    pub(super) async fn take_woken_marks_in_op(
        &self,
        op: &mut (impl es_entity::AtomicOperation + ?Sized),
        ids: &[uuid::Uuid],
    ) -> Result<Vec<JobId>, JobError> {
    ...
    pub(super) async fn delete_waits_of_in_op(
        &self,
        op: &mut (impl es_entity::AtomicOperation + ?Sized),
        ids: &[uuid::Uuid],
    ) -> Result<(), JobError> {
        sqlx::query!("DELETE FROM job_waiters WHERE waiter_job_id = ANY($1)", ids)
            .execute(op.as_executor())
            .await?;
        Ok(())
    }
    ...
    /// addressed.
    pub(super) async fn pull_forward_ids_in_op(
        &self,
        op: &mut (impl es_entity::AtomicOperation + ?Sized),
        ids: &[uuid::Uuid],
